...
const float ntctemp_lookuptable[ntctemp_lookuptablesize] = {
...
float ntctemp_getLookup(uint16_t adcvalue) {
	...
	maxt = (ntctemp_lookuptable[i]); //highest interval value
	if(i==0)
		mint = maxt;
	else
		mint = (ntctemp_lookuptable[i-1]); //smallest interval value
